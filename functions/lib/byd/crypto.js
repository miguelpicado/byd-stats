...
exports.aesEncryptHex = aesEncryptHex;
exports.aesDecryptUtf8 = aesDecryptUtf8;
exports.buildSignString = buildSignString;
exports.loadBangcleTables = loadBangcleTables;
exports.areBangcleTablesLoaded = areBangcleTablesLoaded;
...
function pwdLoginKey(password) {
    ...
    console.log(`[pwdLoginKey] password length: ${password === null || password === void 0 ? void 0 : password.length}, firstHash: ${firstHash} (${firstHash.length}), secondHash: ${secondHash} (${secondHash.length})`);
    return secondHash;
}
/**
 ...
 */
function aesEncryptHex(plaintext, keyHex) {
    console.log(`[aesEncryptHex] keyHex received: "${keyHex}" (length=${keyHex === null || keyHex === void 0 ? void 0 : keyHex.length})`);
    // Validate key is 32 hex chars (16 bytes)
    if (!keyHex || keyHex.length !== 32 || !/^[0-9A-Fa-f]+$/.test(keyHex)) {
        throw new Error(`Invalid AES key: expected 32 hex chars, got "${keyHex}" (length=${keyHex === null || keyHex === void 0 ? void 0 : keyHex.length})`);
    }
 ...
 */
function aesDecryptUtf8(ciphertextHex, keyHex) {
    const key = Buffer.from(keyHex, 'hex');
 ...
 */
function areBangcleTablesLoaded() {
 ...
 */
function decryptBlockAuth(block, roundStart = 1) {
    if (!bangcleTables)
        throw new Error('Bangcle tables not loaded');
 ...
 */
function encryptBlockAuth(block, roundEnd = 10) {
    if (!bangcleTables)
        throw new Error('Bangcle tables not loaded');
