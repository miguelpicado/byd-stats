...
import { STORAGE_KEY, TRIP_HISTORY_KEY, CHARGES_STORAGE_KEY, SETTINGS_KEY, uuid } from '@core/constants';
...
const safeSetItem = (key: string, value: string): boolean => {
    try {
        ...
        logger.warn('localStorage write failed', { key, error: String(e) });
        return false;
    }
};
...
export const CarProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
        ...
        const initialize = () => {
            ...
            if (cars.length === 0) {
                ...
                if (legacySettings || legacyStats) {
                    ...
                    if (legacySettings) safeSetItem(`${SETTINGS_KEY}_${newCarId}`, legacySettings);
                    if (legacyStats) safeSetItem(`${STORAGE_KEY}_${newCarId}`, legacyStats);
                    if (legacyCharges) safeSetItem(`${CHARGES_STORAGE_KEY}_${newCarId}`, legacyCharges);
                    if (legacyHistory) safeSetItem(`${TRIP_HISTORY_KEY}_${newCarId}`, legacyHistory);

                    // Save new state
                    safeSetItem(CARS_STORAGE_KEY, JSON.stringify(newCars));
                    safeSetItem(ACTIVE_CAR_KEY, newCarId);
                ...
                safeSetItem(ACTIVE_CAR_KEY, cars[0].id);
            }
        };
    ...
    useEffect(() => {
        if (cars.length > 0) {
            safeSetItem(CARS_STORAGE_KEY, JSON.stringify(cars));
        }
    }, [cars]);

    useEffect(() => {
        if (activeCarId) {
            safeSetItem(ACTIVE_CAR_KEY, activeCarId);
        }
    }, [activeCarId]);
