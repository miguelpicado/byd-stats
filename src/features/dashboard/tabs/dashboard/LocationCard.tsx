...
// Lazy load leaflet and map components to reduce main bundle size
const LazyMapComponents = React.lazy(() => import('@components/maps/LocationCardMap'));
    ...
    return (
        <div className="w-full flex-1 min-h-[120px] bg-white dark:bg-slate-800/40 rounded-2xl border border-slate-200 dark:border-slate-700/30 overflow-hidden relative group shadow-sm dark:shadow-none">
            {hasLocation ? (
                ...
                >
                    <React.Suspense fallback={<div className="h-full w-full bg-slate-100 dark:bg-slate-800 animate-pulse" />}>
                        <LazyMapComponents
                            mapCenter={mapCenter}
